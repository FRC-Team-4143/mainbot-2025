...
public class ReefSectionState implements Logged {
  ...
  public static ReefSectionState averageReefSections(
    ...
    double[] left_column_count = {0, 0, 0}, right_column_count = {0, 0, 0}, algae_count = {0, 0};
    ...
    for (int i = 0; i < left_column_count.length; i++) {
      final_reef_section_state.left_column[i] =
          (left_column_count[i] / left_column_count.length) < threshold;
      final_reef_section_state.right_column[i] =
          (right_column_count[i] / right_column_count.length) < threshold;
    }

    for (int j = 0; j < algae_count.length; j++) {
      final_reef_section_state.algae[j] = (algae_count[j] / algae_count.length) < threshold;
    }
