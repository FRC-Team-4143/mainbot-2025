...
public class Elevator extends Subsystem {
  ...
  public void updateLogic(double timestamp) {
    ...
    switch (io_.pending_target.getControlType()) {
      case EFFECTOR:
        io_.target_elevator_height_ = kinematics_.desiredElevatorZ(io_.pending_target.getHeight(),
  ...
  public class ElevatorPeriodicIo implements Logged {
    ...
    @Log.File
    public ControlType current_control_mode_ = ControlType.PIVOT;
    @Log.File
    ...
    @Log.File
    public Rotation2d target_arm_angle_ = TargetType.CORAL_INTAKE.getTarget().getAngle();
    @Log.File
    public TargetType target_type_ = TargetType.CORAL_INTAKE;
    @Log.File
    public ArrayList<TargetData> intermediate_targets_ = new ArrayList<>();
    @Log.File
    public double elevator_master_rotations_ = 0;
    @Log.File
    public double elevator_follower_rotations_ = 0;
    @Log.File
    public TargetData pending_target = new TargetData();
    // @Log.File public TargetData target_data_ = target_.getLoggingObject();
    @Log.File
    public SpeedLimit current_speed_limit;
  }
