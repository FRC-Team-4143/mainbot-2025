...
public class GameStateManager extends Subsystem {
    ...
    switch (io_.robot_state_) {
      ...
      SwerveDrivetrain.getInstance().setTargetPose(io_.reef_target.get());
        if (Util.epislonEquals(
            ...
            Constants.GameStateManagerConstants.REQUIRED_ROTATION_FOR_ELEVATOR)) { // move elevator once within rotation
  ...
  public void elevatorTargetSwitch() {
    switch (io_.scoring_target) {
      ...
      case L2:
        ...
        break;
      case L2_HIGH:
        Elevator.getInstance().setTarget(TargetType.L2_HIGH);
        break;
      case L3:
  ...
  public void updateNeededCoralOffset() {
    if (io_.reef_target.isPresent() && SwerveDrivetrain.getInstance().getFailingToReachTarget() && SwerveDrivetrain
        .getInstance().getTractorBeamError() <= Constants.GameStateManagerConstants.CORAL_BLOCKED_THRESHHOLD) {
